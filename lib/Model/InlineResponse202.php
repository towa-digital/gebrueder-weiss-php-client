...
class InlineResponse202 implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setOrderId($order_id)
    {
        if (!is_null($order_id) && (mb_strlen($order_id) > 10)) {
            throw new \InvalidArgumentException('invalid length for $order_id when calling InlineResponse202., must be smaller than or equal to 10.');
        }
        if (!is_null($order_id) && (mb_strlen($order_id) < 10)) {
            throw new \InvalidArgumentException('invalid length for $order_id when calling InlineResponse202., must be bigger than or equal to 10.');
        }
        if (!is_null($order_id) && (!preg_match("/[0-9A-Z]{10}/", $order_id))) {
            throw new \InvalidArgumentException("invalid value for $order_id when calling InlineResponse202., must conform to the pattern /[0-9A-Z]{10}/.");
        }
